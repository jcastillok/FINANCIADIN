...
#Region "CONTROLES Y EVENTOS"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Me.IsPostBack = False Then
            deFecDep.Value = System.DateTime.Now
    ...
    Protected Sub deFecDep_DateChanged(sender As Object, e As EventArgs) Handles deFecDep.DateChanged
    ...
    Protected Sub cbLiquida_CheckedChanged(sender As Object, e As EventArgs) Handles cbLiquida.CheckedChanged
        ...
        If cbLiquida.Checked Then
            ...
            While j < tablaAmort.Rows.Count
                If IsDBNull(tablaAmort.Rows.Item(j).Item(2)) And (tablaAmort.Rows.Item(j).Item(0) <= (credito.NumPagos - 3)) Then
                    pagosLiquid.Add(tablaAmort.Rows.Item(j).Item(3))
                ElseIf IsDBNull(tablaAmort.Rows.Item(j).Item(2)) And (tablaAmort.Rows.Item(j).Item(0) > (credito.NumPagos - 3)) Then
    ...
    Protected Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        ...
        If IsNothing(Session("ProcesandoPago")) Then
            ...
            Else
                ...
                Dim ultimoPago As Int32 = Convert.ToInt32(lblPago.Text)
                If tablaPagosAux.Count = 0 And usuario.Tipo <> 10 And usuario.Tipo <> 1 Then
                    Utilerias.MensajeAlerta("No cuenta con permisos para capturar la garantía", Me, True)
                    Session.Remove("ProcesandoPago")
                Else
                    guardarPago()
                End If
    ...
    Protected Sub btnIngresar_Click(sender As Object, e As EventArgs) Handles btnIngresar.Click
        ...
        If IsNothing(clUsuario) = False AndAlso ValidaPassword(cmd, rst) AndAlso (clUsuario.Tipo = 1 Or clUsuario.Tipo = 9 Or clUsuario.Tipo = 10) Then
...
#Region "METODOS DE LA CLASE"
        ...
        If cbClientes.SelectedIndex <> -1 And cbIdCredito.SelectedIndex <> -1 Then
            ...
            Else
                ...
                While j < tablaAmort.Rows.Count
                    ...
                    Else
                        If IsDBNull(tablaAmort.Rows.Item(j).Item(2)) And tablaAmort.Rows.Item(j).Item(1) <= deFecDep.Value.Date And esperadoGuardado Then
                ...
                Session("Recargo") = calcularRecargo(credito, deFecDep.Value.Date, fechaEsperada)
    ...
    Private Sub guardarPago()
        ...
        Try
            ...
            'If IsNothing(pagoCancelado) AndAlso IsNothing(ultPagoObligatorio) = False AndAlso deFecDep.Value > ultPagoObligatorio.FecProxPago And cbAbonoCap.Checked Then Utilerias.MensajeAlerta(String.Format("No puedo hacer un abono a capital con un saldo vencido!", camposVacios), Me, True) : Return
            Dim historial As List(Of clsPago) = Session("Historial")
            ...
            '.FecEsperada = If(Session("EsPagoGarantia"), deFecDep.Value, Utilerias.calcularFechaDePago(credito.FecInicio, lblPago.Text, credito.Plazo)),
            '.FecPago = deFecDep.Value,
            ...
            '.Atrasado = If(deFecDep.Value <= Utilerias.calcularFechaDePago(credito.FecInicio, lblPago.Text, credito.Plazo), False, True),
            ...
            pago.FecEsperada = If(Session("EsPagoGarantia") Or cbLiquidaBase.Checked Or cbAbonoCap.Checked, deFecDep.Value.Date, Utilerias.calcularFechaDePago(credito.FecInicio, lblPago.Text, credito.Plazo))
            'If IsNothing(pagoCancelado) Then
            pago.FecPago = deFecDep.Value.Date
            pago.Atrasado = If(cbLiquidaBase.Checked Or cbAbonoCap.Checked Or Session("EsPagoGarantia") = True, False, If(deFecDep.Value.Date <= Utilerias.calcularFechaDePago(credito.FecInicio, lblPago.Text, credito.Plazo), False, True))
            ...
            pago.UltModif = deFecDep.Value
            pago.LoginModif = Session("Usuario").Login
            pago.FecProxPago = If(cbLiquidaBase.Checked Or saldoCap <= 0, CType(Nothing, DateTime?), If(cbAbonoCap.Checked, deFecDep.Value.Date, Utilerias.calcularFechaDePago(credito.FecInicio, lblPago.Text + 1, credito.Plazo)))
            pago.Monto = monto
    ...
    Private Sub guardarPagoProgresivo()
        ...
        Try
            ...
            pago.Id_Sucursal = cbSuc.SelectedItem.Value
            pago.NumPago = If(cbLiquidaBase.Checked And pagosAdicionales.Count > 3, Session("PagoActual"), lblPago.Text)
            pago.FecEsperada = If(cbLiquidaBase.Checked Or cbAbonoCap.Checked, deFecDep.Value.Date, Utilerias.calcularFechaDePago(credito.FecInicio, lblPago.Text, credito.Plazo))
            pago.FecPago = deFecDep.Value.Date
            pago.Atrasado = False
            pago.Login = Session("Usuario").Login
            pago.UltModif = deFecDep.Value
    ...
    Private Function CrearPagoConSaldo(credito As clsCredito, numPago As Integer, monto As Double, id_movto As Integer, Optional esAbCap As Boolean = False) As clsPago
        ...
        pago.FecPago = deFecDep.Value.Date
        pago.Atrasado = If(esAbCap, False, If(deFecDep.Value.Date <= fecEsperada, False, True))
        pago.Login = Session("Usuario").Login
        pago.UltModif = System.DateTime.Now
        pago.LoginModif = Session("Usuario").Login
        pago.FecProxPago = If(esAbCap, deFecDep.Value.Date, Utilerias.calcularFechaDePago(credito.FecInicio, numPago + 1, credito.Plazo))
        pago.Monto = monto
